...
app.get("/jokes/:id",(req,res)=>{
  ...
  console.log(id);
  //find goes through array and finds matching id. The joke paramater is just a name for each object
  //could also use for or foreach
  var joke = jokes.find((joke)=>joke.id===id);
  res.json(joke);
});
...
app.get("/filter",(req,res)=>{
  ...
  console.log(filter);
  function filterJokes(joke){
    if(joke.jokeType === filter){
      foundJokes.push(joke);
    }
  }
  jokes.forEach(filterJokes);
  res.json(foundJokes);
});
...
app.patch("/jokes/:id",(req,res)=>{
  ...
  var id = parseInt(req.params.id);
  const searchIndex = jokes.findIndex((joke) => joke.id === id);
  var existingJoke = jokes[searchIndex];
  var newJoke = {
    id:id,
